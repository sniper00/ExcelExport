...
TypeTraits["raw[]"] ={
    ...
    decode = function(data, filename, colname)
        ...
        if not ok then
            console.error(strfmt("Excel file '%s' col '%s' unsupport data format, maybe server not need this field", filename, colname))
            return data
        end
        return setmetatable({"{"..data.."}"},custommt)
    end
...
local function FormatOne(rawdata, filename)
    ...
    for i=startline,#rawdata do
        ...
        if row[1] then
                ...
                if name then
                    if (not options[idx]) then
                        ...
                        if trait and trait.decode then
                            ...
                            if not ok then
                                console.error(strfmt("Excel file '%s' col '%s' rawdata %s format error: %s", filename, name, tostring(value), res))
                            else
            ...
            if key then
                if resTb[key] then
                    console.error(strfmt("Excel file '%s' row %d key duplicate!", filename, i))
                else
...
for name, data in pairs(rawdatas) do
    ...
    if res then
        ...
        console.warn(strfmt("Excel file '%s' main key is null, will skipped!", name))
    end
end
    ...
    if not direct then
        ...
        for k, v in ipairs(order) do
            if not v or not datatype[k] then
                console.warn(strfmt("Excel file '%s' col '%s' has unsupport datatype '%s' ,skipped!", name, v, datatype[k]))
            else
                if comments[k] then
                    if not options or options[k] ~= "options" then
                        ...
                        result[#result+1] = strfmt("---@field public %s %s @%s", v, TypeTraits[datatype[k]].luat, strgsub(comments[k],"%c",""))
                    end
                else
                    console.warn(strfmt("Excel file '%s' col '%s' has no comments.", name, v, datatype[k]))
